...
export type ScaleKeyPosition = 'bottom-left' | 'bottom-right' | 'top-left' | 'top-right';

export interface MagneticTrajectoryOptions {
    /** Maximum number of points to keep (default: 200) */
    maxPoints?: number;
    /** Visual scale factor (default: 0.35) */
    scale?: number;
    /** Auto-normalize to fit bounds (default: false, uses fixedBounds) */
    autoNormalize?: boolean;
    /** Fixed bounds in µT (default: 200 based on SNR analysis) */
    fixedBounds?: number;
    /** Line color (default: '#4ecdc4') */
    trajectoryColor?: string;
    /** Show start/end markers (default: true) */
    showMarkers?: boolean;
    /** Show bounding cube (default: true) */
    showCube?: boolean;
    /** Canvas background color (default: null for transparent) */
    backgroundColor?: string | null;
    /** Minimum alpha for oldest points (default: 0, fully transparent) */
    minAlpha?: number;
    /** Maximum alpha for newest points (default: 1, fully opaque) */
    maxAlpha?: number;
    /** Show scale bar and units legend (default: true) */
    showScaleKey?: boolean;
    /** Position: 'bottom-left', 'bottom-right', 'top-left', 'top-right' (default: 'bottom-left') */
    scaleKeyPosition?: ScaleKeyPosition;
}
...
export class MagneticTrajectory {
    ...
    private autoNormalize: boolean;
    private fixedBounds: number | null;
    private trajectoryColor: string;
    private showMarkers: boolean;
    private showCube: boolean;
    private backgroundColor: string | null;
    private minAlpha: number;
    private maxAlpha: number;
    private showScaleKey: boolean;
    private scaleKeyPosition: ScaleKeyPosition;
    ...
    constructor(canvas: HTMLCanvasElement, options: MagneticTrajectoryOptions = {}) {
        this.canvas = canvas;
     ...
     * @param mx - X component (µT)
     * @param my - Y component (µT)
     * @param mz - Z component (µT)
     */
    addPoint(mx: number, my: number, mz: number): void {
        // Skip invalid points
     ...
     */
    private normalizePoints(): Point3D[] {
        ...
        return this.points.map(p =>
            p.map((v, i) => ((v - min[i]) / range[i]) * 2 - 1) as Point3D
        );
    }
     ...
     * @param normalizedPoints - Points normalized to [-1, 1]
     */
    private drawTrajectory(normalizedPoints: Point3D[]): void {
     ...
     * @returns Stats about current trajectory
     */
...
export default MagneticTrajectory;
