...
export interface GeomagneticLocationWithDistance extends GeomagneticLocation {
  distance: number;
}
...
export interface BrowserLocationResult {
  ...
  accuracy: number;
  timestamp: number;
  nearest: GeomagneticLocationWithDistance[];
  selected: GeomagneticLocationWithDistance;
}
...
export interface GeomagneticFieldMetadata {
  declination: number;
  declination_unit: string;
  inclination: number;
  inclination_unit: string;
  total_intensity: number;
  horizontal_intensity: number;
  vertical_intensity: number;
...
export interface LocationMetadata {
  ...
  latitude: number;
  longitude: number;
  geomagnetic_field: GeomagneticFieldMetadata;
}
 ...
 */
export const GEOMAGNETIC_LOOKUP: GeomagneticLocation[] = [
    // United Kingdom
 ...
 */
export function findNearestLocations(lat: number, lon: number, maxResults: number = 5): GeomagneticLocationWithDistance[] {
 ...
 * @returns Location object with lat, lon, and nearest lookup entry
 */
export function getBrowserLocation(options: GeolocationOptions = {}): Promise<BrowserLocationResult> {
 ...
 */
export function getLocationByCity(cityName: string): GeomagneticLocation | null {
 ...
 * @returns Array of locations in that country
 */
export function getLocationsByCountry(country: string): GeomagneticLocation[] {
 ...
 * @param location - Location object from lookup
 * @returns Formatted string
 */
export function formatLocation(location: GeomagneticLocation | null | undefined): string {
 ...
 */
export function formatFieldData(location: GeomagneticLocation | null | undefined): FormattedFieldData | null {
 ...
 * @param location - Location object from lookup
 * @returns Metadata-ready object
 */
export function exportLocationMetadata(location: GeomagneticLocation | null | undefined): LocationMetadata | null {
    if (!location) return null;
